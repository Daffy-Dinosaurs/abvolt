...
worldGlobe.go = function (countryObject) {
  ...
  if (!worldGlobe.loaded) {
    ...
    projection = d3.geo.orthographic(3).scale(300).translate([width / 2 + 10, height / 2]).clipAngle(90).precision(0.3);

    path = d3.geo.path().projection(projection);

    svg = d3.select(".globe").append("svg").attr("width", "900").attr("height", "720");
    g = svg.append("g");

    g.append("path").datum({ type: "Sphere" }).attr("class", "sphere").attr("d", path).attr("fill", "#1C6BA0").attr("transform", "translate(0, -20)");

    worldPath = svg.selectAll("path.land").data(countries).enter().append("path").attr("class", "land").attr("d", path).attr("fill", "#C1B398").attr("transform", "translate(0, -20)");
  ...
  function ready(error, world) {
    ...
    // Drag event
    worldPath.call(d3.behavior.drag().origin(function () {
...
worldGlobe.renderGlobeStats = function (storage, lowrange, highrange, category) {
  ...
  let colorScale = d3.scale.linear().domain([lowrange, highrange]).rangeRound([0, 14]).nice();
  ...
  function addingColor(array) {
    for (var i = 0; i < storage.length; i++) {
      ...
      svg.selectAll('path').classed('.focused', function (d, j) {
